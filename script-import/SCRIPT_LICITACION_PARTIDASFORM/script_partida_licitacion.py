...
query = ("SELECT * FROM `partidas_licitadas` WHERE id > 1872 order by partidas_licitadas.id asc;")

cursor.execute(query)

for row in cursor:
    if str(row['tipo_procedimiento']) == 'AD':
        print('es adj')
    else:

        print('la id es: ', row['id'], ' con su licitacion:', row['referencia'])

        Id_obraprogramada = row['clave_partida']

        id_sideop_licitacion = row['referencia']
    ...
    #           print("Si encontro la adjudicacion")
            ...
            print("No pudo encontrar la adjudicacion en la base de datos id: " + str(row['id']) + " id_partida: " + str(row['clave_partida']))
        ...
        for i in b_eve.contratista_propuesta:
            if i.posicion == '1':
                print('es el ganador xxxxxxxxxx', i.name.name)

                if id_licitacion_update != False:
                    # propuestab = odoo.env['proceso.contra_propuestas'].browse(id_licitacion_update.id)

                    b_lic = licitacion.browse(search_licitacion)

                    for x in i.programar_obra_licitacion2:
                        if str(x.obra.Id_obraprogramada) == str(row['clave_partida']):
                            ...
                            datos_lic = {
                                'programar_obra_licitacion': [[0, 0,
                                {
                                    ...
                                    'monto_partida': x.monto_partida,
                                }]]
                            }

                            p = b_lic.write(datos_lic)
                ...
                print('no es el mismo ganador', i.name.name)
