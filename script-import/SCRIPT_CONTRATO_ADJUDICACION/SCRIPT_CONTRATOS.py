...
query = ("SELECT * FROM `contratos`") # WHERE tipo_procedimiento = 'LP'
...
for row in cursor:
    ...
    if row['tipo_procedimiento'] == "LP":
        ...
        if int(_search_contrato) == 0:
            ...
            '''adjjjj = odoo.env['proceso.adjudicacion_directa']
            b_adjjj = adjjjj.browse(_search_adjudicacion)'''
    ...
    if row['tipo_procedimiento'] == "AD":
        ...
        if int(_search_contrato) == 0:
            ...
            _search_adjudicacion = odoo.env['proceso.adjudicacion_directa'].search([("numerocontrato", "=", row['referencia'])])
