...
query = ("SELECT * FROM `licitaciones_fallo` where id > 822")
...
for row in cursor:
    ...
    _search_lici = odoo.env['proceso.licitacion'].search(
        [("numerolicitacion", "=", str(row['id_licitacion']))])

    print(_search_lici, 'LICITACION')
    ...
    if str(_search_evento) == '[]':
        ...
        datos_fallo = {  # 'obra': str(partidas.obra.id),
            'contratista_fallo': [[0, 0,{
                'id_sideop': row['id']
                , 'name': b_contra.id
                ...
                , 'ganador': row['ganador']
                , 'monto': row['monto']
                , 'puntos_tecnicos': row['puntos_tecnicos']
                , 'puntos_economicos': row['puntos_economicos']
                , 'observaciones': row['observaciones']
            }]]
        }
        fallox = contrato_id.write(datos_fallo)
    ...
    # CREAR DATOS DE LA TABLA
        ...
        fallox = b_evento.write(datos_fallo)
